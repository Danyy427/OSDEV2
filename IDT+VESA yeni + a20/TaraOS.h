...
#include <stdint.h>
    ...
    void set_background(unsigned char* screen,int color)
    {
        ...
        fill_rect((char*)vm,0,1280,0,1024,color);   
    }
	...
	void drawchar(unsigned char c, int x, int y, int fgcolor, int bgcolor)
	{
		...
		for(cy=0;cy<16;cy++){
			for(cx=0;cx<8;cx++){
				putpixel((char*)0xA0000,x+cx,y+cy-12,mask[cx]&gylph[cy]?fgcolor:bgcolor);
			}
		}
	}		
	
	#define len(x) sizeof(x)/sizeof(x[0])
	void set_background_with_pixel_buffer(int pixel_buffer[])
	{
		for(int i = 0; i < (int)len(pixel_buffer);i++)
		{
	...
	void drawstring(unsigned char *str, int x, int y, int fgcolor, int bgcolor)
	{
		...
		for(int t = 0; t < strlen(str); t++)
		{
			...
			for(cy=0;cy<16;cy++){
				for(cx=0;cx<8;cx++){
					putpixel((unsigned char*)0xA0000,x+cx,y+cy-12,mask[cx]&gylph[cy]?fgcolor:bgcolor);
				}
			}
			x += 8;
		}
