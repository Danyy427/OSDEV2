...
#include <stdint.h>
  ...
  ////////////////////////////////////////////////
    ...
    uint16_t video_modes_offset;
		...
		uint16_t vendorSEG;			// segment:offset to card vendor string
    uint16_t vendorOFF;			// segment:offset to card vendor string
	...
	void drawstring(unsigned char *str, int x, int y, int fgcolor, int bgcolor)
	{
      ...
      set_DTCursor(DTCursor.x + 8, DTCursor.y);
	...
	//
	void printf(unsigned char* str) //bu değişecek daha yeni şeyler eklenecek
	{

      drawstring(str,DTCursor.x,DTCursor.y,0xffffffff,0x00000000);
...
//
// DUR DUR DUR KARISTI BAYA BI KARISTI
//
// SU KEYBOARDA DOKUNMA LTF
//
// DEGISECEK AMA GEREKSIZ SATIRLARI CIKARINCA BOZULUYOR YAPMA
//


	void irq1_handler(interrupt_frame_t *frame)
	{
    if(isWaitingForInput == 0) goto here;
      ...
      if(scan_code == 0x3A) {
        ...
        if(scan_code == 0x1C) {
            if(enterGoesNext == 1)
            {
              lastChar[(DTCursor.y/16)] = DTCursor.x-8;
              set_DTCursor(0, DTCursor.y + 16);
            }
            lastCharacter = 0x10;
            goto here;
        ...
        if(scan_code == 0x2A || scan_code == 0x36) {
          ...
          if(scan_code == 0x0E) {
            ...
            if(DTCursor.x == 0) {
             set_DTCursor( lastChar[(DTCursor.y/16)-1], DTCursor.y - 16 );
             drawchar(' ',DTCursor.x ,DTCursor.y,0xffffff, 0x000000);
             goto here;
            }
        ...
        if(caps == 1) _ch = ch - 32;
			...
			drawchar(_ch,DTCursor.x ,DTCursor.y,0xffffff, 0x000000);

      lastCharacter = _ch;
      if(DTCursor.x + 16 > 1280)
      {
        set_DTCursor(0, DTCursor.y+16);
      }
      else {
  ...
  // Input that returns message and prints a message before. Like python
  char * input(char * msg)
  {
    printf(msg);
    disableINPUT();
